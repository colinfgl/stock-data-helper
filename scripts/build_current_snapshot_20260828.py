...
S=requests.Session(); S.headers.update({'User-Agent':'Mozilla/5.0 stock-data-helper-current-snapshot/1.0'})
...
def main():
    ...
    for ds,start,end in QUERIES:
        ...
        for code in TARGETS:
            ...
            if err: errors.append({'dataset':ds,'data_id':code,'error':err})
            for x in rows:
                y=dict(x); y['_query_id']=code; y['_source_url']=url; allrows.append(y)
            dates=[str(x.get('date','')) for x in rows if x.get('date')]
            coverage.append({'dataset':ds,'data_id':code,'rows':len(rows),'first_date':min(dates) if dates else '','last_date':max(dates) if dates else '','error':err or ''})
    ...
    try:
        ...
        coverage.append({'dataset':'TWSE_MI_INDEX','data_id':'industry','rows':len(rows),'first_date':'2026-08-28','last_date':'2026-08-28','error':''})
        print('TWSE_MI_INDEX',len(rows),'OK',flush=True)
    except Exception as e:
        errors.append({'dataset':'TWSE_MI_INDEX','data_id':'industry','error':repr(e)})

    write_union(OUT/'coverage.csv',coverage); write_union(OUT/'errors.csv',errors)
    report={'targets':len(TARGETS),'errors':len(errors),'coverage_rows':len(coverage),'all_pass':len(errors)==0}
